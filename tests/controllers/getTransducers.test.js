...
describe('getTransducers', () => {
  ...
  test('should get all transducers', async () => {
    Transducer.find.mockResolvedValueOnce(transducerDocuments);
  ...
  test('should fail to query the database', async () => {
    Transducer.find.mockRejectedValueOnce();
    await getTransducers(req, res, next);
    expect(next).toHaveBeenCalledWith(new Error('Could not query from database'));
  });
});
