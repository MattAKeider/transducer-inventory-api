...
describe('createTransducer', () => {
  ...
  test('should create a transducer', async () => {
    Transducer.findOne.mockResolvedValueOnce(null);
  ...
  test('should fail to query the database', async () => {
    Transducer.findOne.mockRejectedValueOnce();
    await createTransducer(req, res, next);
  ...
  test('should throw an error if transducer already exists', async () => {
    Transducer.findOne.mockResolvedValueOnce(transducerDocument);
    await createTransducer(req, res, next);
    expect(next).toHaveBeenCalledWith(new Error('Transducer already exists with given name or serial number'));
  });

  test('should fail to write to the database', async () => {
    Transducer.prototype.save.mockRejectedValueOnce();
    
    await createTransducer(req, res, next);
    expect(next).toHaveBeenCalledWith(new Error('Failed to write to database.'));
  });
});